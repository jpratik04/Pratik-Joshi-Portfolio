...
// Sub-component to manage individual refs and staggered delays per item
const ExperienceCard = ({ exp, index }: { exp: any; index: number }) => {
  ...
  return (
    <div
      ref={cardRef}
      className={`relative pl-8 md:pl-24 transition-all duration-1000 ${
        cardInView ? "opacity-100 translate-x-0" : "opacity-0 translate-x-20"
      }`}
      style={{ transitionDelay: `${(index + 1) * 200}ms` }} // Staggers each card (200ms, 400ms, etc.)
    >
      {/* Timeline dot */}
...
const Experience = () => {
  ...
  const experiences = [
    ...
    },
    {
      ...
      description:
        "Created user-centered designs for web and mobile applications. Conducted user research and usability testing to inform design decisions. Worked closely with development teams to ensure seamless implementation of design concepts and maintained consistency across multiple projects.",
    },
  ];
